...
class GameLogic1006DrawFront(SpriteNumber: Int, BackTileNumber: Int) extends Module {
  ...
  //states
  val idle :: compute1 :: compute2:: compute3 ::done :: Nil = Enum(4)
    ...
    is(compute2){
      when(viewBoxXReg === (xBoxReg*32.U)) {
        when(yBoxReg < 15.U){
          ...
          io.backBufferWriteAddress := 20.U+xBoxReg+(yBoxReg*40.U) // box mapping
